...
def trouver_indicateur_euler(n:int, p:int, M:int):
    """
    Question associée:
        a) calculer N = np puis Phi(N) étant l'indicateur d'euler et vérifier M (étant le message en clair) XOR N.
    Démarche:
        N = n*p
        Phi(n*p) = (n-1) * (p-1)  
    Exemple:
        N = 7*13 
        <=> Phi(7*13) = (7-1) * (13-1) = 72
    """

    phi_n = (n-1)*(p-1) 
    if M in init_nb_premiers(M):
        return phi_n, 1
    else:
        return phi_n, "pas premier" 
...
def inverse_modulaire_bezout(phi_n:int, e:int):
    """
        Question associée:
            c) Déterminer l'entier naturel d < Phi(N) tel que ed≡1[Phi(N)]
        Démarche: 
            Ligne1 = A un entier naturel ainsi que 1*A  et 0*B = Phi(N) dans la question associée
            Ligne2 = B un entier naturel ainsi que 0*A et 1*B = e dans la question associée
            ...
            On prend l'avant dernière ligne, soit le résultat de Ligne1 puisque Ligne2 aura un résultat = 0, le PGCD doit être non nul, donc seul la ligne antérieur nous intéresse.
